var Rendering;
(function (Rendering) {
    function renderTriangles(ctx, triMesh, size, cameraAndScale) {
    ...
    Rendering.renderTriangles = renderTriangles;

    function renderOutlining(ctx, triangleRendering) {
        ...
        for (var i = 0; i < triMesh.edgeStarts.length; i++) {
            ...
            function cameraDotProduct(normalIndex) {
            ...
            if (shouldRenderHardEdge || shouldRenderSoftEdge) {
                ...
                if (spanX >= spanY) {
                    ...
                    for (var x = psx; x <= pex; x++) {
                        ...
                        var actualDepth = Math.min(depthBuffer[(outputPtr) >> 2], depthBuffer[(outputPtr + width * 4) >> 2], depthBuffer[(outputPtr + 4) >> 2], depthBuffer[(outputPtr - width * 4) >> 2], depthBuffer[(outputPtr - 4) >> 2]);
    ...
    Rendering.renderOutlining = renderOutlining;

    (function (Diagnostics) {
        function renderTriangles(ctx, render) {
        ...
        Diagnostics.renderTriangles = renderTriangles;

        function renderDepthBuffer(ctx, render) {
            var width = render.size.width, height = render.size.height;
        ...
        Diagnostics.renderDepthBuffer = renderDepthBuffer;

        function renderTrianglesToCanvas(ctx, triMesh, size, cameraAndScale) {
        ...
        Diagnostics.renderTrianglesToCanvas = renderTrianglesToCanvas;

        function renderEdgesToCanvas(ctx, renderResult) {
        ...
        Diagnostics.renderEdgesToCanvas = renderEdgesToCanvas;
    })(Rendering.Diagnostics || (Rendering.Diagnostics = {}));
    var Diagnostics = Rendering.Diagnostics;
})(Rendering || (Rendering = {}));
