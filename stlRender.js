...
(function (View) {
    View[View["Top"] = 0] = "Top";
    View[View["Bottom"] = 1] = "Bottom";
    View[View["Left"] = 2] = "Left";
    View[View["Right"] = 3] = "Right";
    View[View["Front"] = 4] = "Front";
    View[View["Back"] = 5] = "Back";
})(View || (View = {}));

var OrthoCameraSet = (function () {
    function OrthoCameraSet(extents, drawWidth, drawHeight) {
        ...
        function makeCameraExtents(xSize, ySize, zSize) {
        ...
        this.cameras[0 /* Top */] = new SimpleOrthographicCamera(scale, scale, 0, 0, scale, 0, 0, 0, 0, -1, makeCameraExtents(cameraWidth, cameraHeight, undefined));

        this.cameras[4 /* Front */] = new SimpleOrthographicCamera(scale, scale, 0, 0, 0, 0, scale, 0, 1, 0, makeCameraExtents(cameraWidth, undefined, cameraHeight));

        this.cameras[2 /* Left */] = new SimpleOrthographicCamera(scale, 0, 0, scale, 0, 0, scale, -1, 0, 0, makeCameraExtents(undefined, cameraWidth, cameraHeight));
    }
    return OrthoCameraSet;
})();

var SimpleOrthographicCamera = (function () {
    function SimpleOrthographicCamera(/** Scale factor in pixels per world unit */
    scale, /** Vector contribution of changes in world x to change in camera y (including scale factor) */
    xx, xy, yx, yy, zx, zy, /** Direction of camera rays */
    xd, yd, zd, extents) {
        this.scale = scale;
        this.xx = xx;
        this.xy = xy;
    ...
    SimpleOrthographicCamera.prototype.worldToFilm = function (v) {
    ...
    SimpleOrthographicCamera.prototype.depthOf = function (v) {
        var vx = v.x - this.extents[0].x;
        var vy = v.y - this.extents[0].y;
        var vz = v.z - this.extents[0].z;
    ...
    SimpleOrthographicCamera.prototype.filmToWorld = function (p) {
        var vx = 0, vy = 0, vz = 0;
        if (this.xx !== 0)
            vx += p.x / this.xx;
        if (this.xy !== 0)
        ...
        result.set(vx, vy, vz);
        return new THREE.Ray(result, new THREE.Vector3(this.xd, this.yd, this.zd));
    };
    return SimpleOrthographicCamera;
})();

function renderOutline(depthBuffer, width, height, output) {
    ...
    for (var x = 0; x < width; x++) {
        ...
        for (var y = 0; y < height; y++) {
            var i = (y * width + x) << 2;
            var di = i >> 2;
            var diff = Math.min(Math.abs(prev - depthBuffer[di] - delta), Math.abs(prev - depthBuffer[di] - delta * 1.1), Math.abs(prev - depthBuffer[di]));
            ...
            if (delta !== delta)
                delta = 0; // Infinity - Infinity case
            prev = depthBuffer[di];
        }
    }
    ...
    for (var y = 0; y < height; y++) {
        ...
        for (var x = 0; x < width; x++) {
            var i = (y * width + x) << 2;
            var di = i >> 2;
            var diff = Math.min(Math.abs(prev - depthBuffer[di] - delta), Math.abs(prev - depthBuffer[di] - delta * 1.1), Math.abs(prev - depthBuffer[di] - delta / 1.1), Math.abs(prev - depthBuffer[di]));
...
function renderGrid(origin, pixelsPerWorldUnit, width, height, output) {
    ...
    var startX = origin.x;
    var startY = origin.y;
    while (startX > pixelSpacing)
        startX -= pixelSpacing;
    while (startX < -pixelSpacing)
        startX += pixelSpacing;
    while (startY > pixelSpacing)
        startY -= pixelSpacing;
    while (startY < -pixelSpacing)
        startY += pixelSpacing;
...
function renderSTL(stlData, width, height, createImageData, view) {
...
//# sourceMappingURL=stlRender.js.map
